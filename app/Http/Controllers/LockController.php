...
class LockController extends Controller
{
    function ruleSetting(Request $request)
    {
        ...
        switch ($request->type) {
            case 'chapters':
                ...
                break;
            default:
                return response('Zadán špatný název tabulky elementu ' . $request->type, 500)->header('Content-Type', 'text/plain');
    ...
    function saveRule(Request $request)
    {
        ...
        $data = DB::table($request->table_type)->where('id', $request->id)->get();

        if($data[0]->key == $request->key && $data[0]->security == $request->key_type  ){
            return "1";
        }

        $check = DB::table($request->table_type)->where('id', $request->id)->update(['key' => $request->key, 'security' => $request->key_type ]);

        if(!$check){
            return response('Nastal problém při ukládání dat do databáze' . $request->type, 500)->header('Content-Type', 'text/plain');
        }
    ...
    function checkLock(Request $request)
    {
        ...
        $check_data = DB::table($request->type)->where('id', $request->id)->get();

        if(count($check_data) == 0 && count($check_data) > 1){
            return response('Duplicitní položky v tabulce ' . $request->type, 500)->header('Content-Type', 'text/plain');
        }
        ...
        $check_locks = DB::table('locks')->Join('users', 'locks.user_id', '=', 'users.id')->where('table_name', $request->type)->where('element_id', $request->id)->where('users.id', Auth::user()->id)->select('locks.table_name', 'locks.element_id', 'locks.locked', 'locks.created_at', 'locks.updated_at')->get();

        if(count($check_locks) == 0){
            $lock = new Locks;
            $lock->table_name = $request->type;
            ...
            if(!$check_lock) {
                return response('Došlo k problému při ukládání dat do tabulky locks ' . $request->type, 500)->header('Content-Type', 'text/plain');
            }
            ...
            switch ($check_data[0]->security) {
                ...
                case 'time':
                    ...
                    if(intval($check_data[0]->key) - $now->diffInSeconds($before) <= 0){
                        ...
                        DB::table('locks')->Join('users', 'locks.user_id', '=', 'users.id')->where('table_name', $request->type)->where('element_id', $request->id)->where('users.id', Auth::user()->id)->update(['locked' => '0']);
                        return array("1");
                    }else{
    ...
    function unlock(Request $request)
    {
        ...
        $checked = DB::table('locks')->leftJoin('users', 'locks.user_id', '=', 'users.id')->where('table_name', $request->type)->where('element_id', $request->id)->where('users.id', Auth::user()->id)->update(['locked' => '0']);
