...
>
    ...
    @endif
        ...
        @endphp
        <button class="su-button text-center p-3 w-100 mx-auto d-grid finish-test-button" @if(!$edit && $element->data != null)onclick="finishTest(this.parentNode, '{{$element->data}}', this.getElementsByClassName('loading')[0], this.getElementsByClassName('loading_request')[0])"@endif>
        ...
        @if(!$test_results)
            <div class="check-results w-100 d-block rounded mt-2"  style="    height: 4rem;">
                ...
                @endphp
                <a href="/chapter/{{$chapter->id}}/all_results/{{$element->id}}" class="text-decor-none w-content mx-auto mx-md-0 float-md-end text-su-blue fw-bold d-grid text-center bg-su-blue-gradient rounded-3 shadow p-3 su-animation-02 cursor-pointer su-hover-shadow text-su-shadow-white">
